...
const sendImageDataUrlToPrint = async (p) => {
    try {
...
const sendImageDataUrlsForBackup = async (p) => {
    try {
        const controller = new AbortController();
        setTimeout(() => controller.abort(), 3000);
...
const retrieveBackupImageDataUrls = async (k) => {
    try {
        const controller = new AbortController();
        setTimeout(() => controller.abort(), 3000);
...
const retrieveBackupImageList = async () => {
    try {
        const controller = new AbortController();
        setTimeout(() => controller.abort(), 3000);
...
const retrievePrintQueue = async () => {
    try {
        const controller = new AbortController();
        setTimeout(() => controller.abort(), 3000);
        ...
        return parseResponse.success && parseResponse.data.success
            ? { success: true, data: parseResponse.data.data }
            : { success: false, data: 'print queue response does not match expected data structure' };
    }
    catch (e) {
        return { success: false };
    }
};
const retrieveAvailableDiskSpace = async () => {
    console.log(`sendImageDataUrlToPrint.ts:${ /*LL*/95}`, { 123: 13 });
    try {
        const controller = new AbortController();
        setTimeout(() => controller.abort(), 3000);
        ...
        const json = await response.json();
        console.log(`sendImageDataUrlToPrint.ts:${ /*LL*/105}`, { json });
        const parseResponse = z.string().safeParse(json);
        return parseResponse.success
            ? { success: true, data: parseResponse.data }
            : { success: false, data: 'available disk space does not match expected data structure' };
    }
    catch (e) {
        return { success: false };
    }
...
const onCountdown = async (start, stop, fn) => {
...
const getImageDataUrlFromVideoElement = (p) => {
...
export { sendImageDataUrlToPrint as a, retrievePrintQueue as b, retrieveAvailableDiskSpace as c, delay as d, retrieveBackupImageDataUrls as e, getImageDataUrlFromVideoElement as g, onCountdown as o, retrieveBackupImageList as r, sendImageDataUrlsForBackup as s };

//# sourceMappingURL=index-146c9291.js.map